<div class="card">
    <div class="card-header">
        <h4>แจ้งชำระเงิน Credit Note</h4>
    </div>
    <div class="card-body">
        <form action="{{route('payment.credit-payment')}}" method="post" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="payment_doc_number" value="{{$creditModel->credit_note_number}}">
            <input type="hidden" name="payment_doc_type" value="credit-note">

          
            <div class="row">
                <div class="col-md-3 mt-3">
                    <label for="">จำนวนเงินที่จะชำระ</label>
                    <input type="number" value="{{ $creditModel->grand_total -  $creditModel->payment }}" name="payment_total"
