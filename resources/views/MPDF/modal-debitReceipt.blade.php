<div class="card">
    <div class="card-body">
        <form id="sendEmailForm" method="post">
            @csrf
            <div class="row">
                <h4>ส่งเมล์ ใบเพิ่มหนี้# {{ $debitModel->debit_number }}</h4>
                <hr>
                <div class="col-md-12 mb-3">
                    <label for="">Subject</label>
                    <input type="text" name="subject" class="form-control" value="ใบเพิ่มหนี้ Next Trip Holiday">
                </div>
                <div class="col-md-12 mb-3">
...
<script>
    document.getElementById("sendEmailForm").addEventListener("submit", function(event) {
        ...
        fetch("{{ route('mpdf.debitReceipt.sendPdf', $debitModel->debit_id) }}", {
                method: "POST",
                headers: {
