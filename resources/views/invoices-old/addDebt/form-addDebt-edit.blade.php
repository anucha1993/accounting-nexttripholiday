<div class="row">
    <h4 class="text-info">ใบเพิ่มหนี้ #{{$debt->debt_number}}</h4>
    ...
    <form id="editDebt-form" method="post">
        @csrf
        @method('PUT')
        <br>
        {{-- <div class="row">
            <div class="col-md-4">
                ...
                <select name="payment_type" class="form-select payment-type" required>
                    <option @if($debt->payment_type === 'full') selected @endif  value="full">ชำระเต็มจำนวน</option>
                    <option @if($debt->payment_type === 'deposit') selected @endif value="deposit">มัดจำ(แบ่งชำระ)</option>
                </select>
            </div>
            <div class="col-md-8">
                <div class="full">
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <input type="datetime-local" name="payment_before_date" 
                                class="form-control" value="{{$debt->payment_before_date}}" required>
                        </div>
                        <div class="col-md-4">
                            <label id="text-deposit">จำนวนเงิน มัดจำ</label>
                            <input type="number" name="deposit" class="form-control deposit text-end"
                               placeholder="00.00" min ="0.01" step="0.01"  value="{{$debt->deposit}}">
                        </div>
                        <div class="col-md-4">
        ...
        <div class="col-md-12">
            <table class="table table-bordered" id="table-product">
                ...
                <tbody>
                     ...
                     @forelse ($debtProduct as $key => $item)
                        ...
                        <tr id="productList" class="mt-0">
                            ...
                            <td><input name="product_qty[]" type="number" value="{{ $item->debt_qty }}"
                                    class="form-control text-end"></td>
                            <td><input name="product_price[]" type="number" value="{{ $item->debt_price }}"
                                    class="form-control text-end"></td>
                            <td><input name="product_sum[]" type="number" value="{{ $item->debt_sum }}"
            ...
            <div class="col-md-6">
                <div class="col-md-12">
                    <input type="checkbox" name="vat_3_status" id="vat3" value="Y"
                        @if ($debt->vat_3_status === 'Y') checked @endif>
                ...
                <div class="col-md-12 mt-3">
                  ...
                  <input type="text" name="debt_add_note" class="form-control" value="{{$debt->debt_add_note}}" placeholder="กรุณาระบุ" required>
                </div>

              
                <div class="col-md-12">
                    <label class="text-danger">หมายเหตุ : </label>
                    <textarea name="debt_note" cols="30" rows="3" class="form-control">{{$debt->debt_note}}</textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="row">
        ...
        <input type="hidden" name="debt_id" id="debt-id" value="{{$debt->debt_id}}">
        ...
        <input type="hidden" name="debt_number"  value="{{ $debt->debt_number }}">
...
$(document).ready(function() {
    $('#editDebt-form').submit(function(event) {
        event.preventDefault(); // ป้องกันการส่งฟอร์มแบบปกติ
        var debID = $('#debt-id').val();
        var form = $(this);
        var formData = form.serialize(); // แปลงข้อมูลฟอร์มเป็นแบบ URL-encoded
        $.ajax({
            url: '{{ url("invoice/adddebt/update") }}/' + debID, // URL ของ endpoint ที่ต้องการส่งข้อมูลไป
            type: 'PUT',
            data: formData,
