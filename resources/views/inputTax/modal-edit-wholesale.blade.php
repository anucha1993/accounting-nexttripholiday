<div class="modal-body">
    ...
    <form action="{{ route('inputtax.update', $inputTaxModel->input_tax_id) }}" method="POST"
        ...
        <div class="row">
            <div class="col-md-12 mb-3">
                ...
                <select name="input_tax_type" id="" class="form-select">
                    <option {{ $inputTaxModel->input_tax_type === 4 ? 'selected' : '' }} value="4">
                        ค่าทัวร์รวมทั้งหมด</option>
                    <option {{ $inputTaxModel->input_tax_type === 2 ? 'selected' : '' }} value="2">ค่าห้อง</option>
                    <option {{ $inputTaxModel->input_tax_type === 5 ? 'selected' : '' }} value="5">ค่าอาหาร
                    </option>
                    <option {{ $inputTaxModel->input_tax_type === 6 ? 'selected' : '' }} value="6">
                        ค่าตั๋วเครื่องบิน</option>
            ...
            <div class="col-md-12 mb-3">
                ...
                <input type="date-local" name="" class="form-control" placeholder="tax number"
                    value="{{ date('d-m-Y : H:m:s') }}" disabled>
            </div>

            <div class="col-md-12 mb-3">
                <label for="">ผลรวมต้นทุน</label>
                <input type="number" name="input_tax_grand_total" step="0.01"
                    value="{{ $inputTaxModel->input_tax_grand_total }}" class="form-control" placeholder="0.0"
                    id="total">
            </div>

            @if ($inputTaxModel->input_tax_file)
                <a href="{{ asset('storage/' . $inputTaxModel->input_tax_file) }}"
                    onclick="openPdfPopup(this.href); return false;"><i class="fa fa-file text-danger"></i> ไฟล์แนบ | {{$inputTaxModel->input_tax_file}}</a>
            @else
                
            @endif
            <div class="col-md-12 mb-3">
                    <label for="">ไฟล์เอกสารแนบ/แนบใหม่</label>
