     ...
     <table class="table mb-0">
         ...
         <tbody>
             @foreach ($quotationSuccess as $item)
                 <tr>
                     ...
                     @php
                    $commission = calculateCommission($item->getNetProfit(), $item->quote_sale, 'all', $item->quote_pax_total);
                 @endphp
                 <td>
         ...
         <tfoot>
            <tr>
                ...
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->quote_grand_total + $item->quote_discount; }), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum('quote_discount'), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum('quote_grand_total'), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->getWholesalePaidNet(); }), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->getTotalOtherCost(); }), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->getTotalCostAll(); }), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->getNetProfit(); }), 2) }}</th>
                <th>{{ number_format($quotationSuccess->sum(function($item) { return $item->getNetProfitPerPax(); }), 2) }}</th>
                <th>CommissionGroup</th>
            </tr>
        </tfoot>
     </table>
