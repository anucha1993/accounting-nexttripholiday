...
class quoteReportController extends Controller
{
    ...
    {
        if($request->ajax()) {
            $data = quotationModel::with('customer','quoteCountry','quoteWholesale','Salename','airline')->get()->map(function($row){
                ...
                $row['travel_name'] = $row->airline ? $row->airline->travel_name : '';
                return $row;
            });
